...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      { title: "Dashboard — Proconect" },
      {
        name: "description",
        content: "Coordonează toate lucrările de instalare fibră optică B2B.",
      },
      { property: "og:title", content: "Dashboard — Proconect" },
      {
        property: "og:description",
        content: "Dashboard de coordonare pentru lucrări fibră optică.",
      },
...
const statusIcon: Record<InstallStatus, React.ReactNode> = {
  draft: <MapPin className="size-4" />,
  in_progress: <MapPin className="size-4" />,
  completed: <MapPin className="size-4" />,
  blocked: <MapPin className="size-4" />,
};
...
function Dashboard() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["installations-dashboard"],
    queryFn: async () => {
  ...
  return (
    <AppShell title="Instalări" subtitle="Coordonare lucrări fibră optică B2B">
      {/* Stats Grid */}
      {!isLoading && data && (
